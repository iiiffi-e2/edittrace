"use strict";(()=>{var p=class{constructor(e){this.config=e}inspect(e,t){return this.post("inspect",e,t)}search(e,t){return this.post("search",e,t)}async post(e,t,r){let i=this.config.restUrl.replace(/\/$/,"")+"/"+e,n=await fetch(i,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json","X-WP-Nonce":this.config.nonce},body:JSON.stringify({traceId:this.config.traceId,element:t}),signal:r}),o=null;try{o=await n.json()}catch{o=null}if(!n.ok){let s=o&&typeof o=="object"&&"message"in o?String(o.message):`HTTP ${n.status}`;throw new Error(s)}return o}};var u=class{constructor(e){this.hoverTarget=null;this.selectedTarget=null;this.hover=document.createElement("div"),this.hover.className="edittrace-highlight edittrace-highlight--hover",this.hover.hidden=!0,this.hoverBadge=document.createElement("span"),this.hoverBadge.className="edittrace-badge",this.hover.appendChild(this.hoverBadge),this.selected=document.createElement("div"),this.selected.className="edittrace-highlight edittrace-highlight--selected",this.selected.hidden=!0,this.selectedBadge=document.createElement("span"),this.selectedBadge.className="edittrace-badge",this.selected.appendChild(this.selectedBadge),e.appendChild(this.hover),e.appendChild(this.selected)}setHover(e,t=""){this.hoverTarget=e,this.hoverBadge.textContent=t,this.draw(this.hover,e)}setSelected(e,t=""){this.selectedTarget=e,this.selectedBadge.textContent=t,this.draw(this.selected,e)}getSelected(){return this.selectedTarget}getHover(){return this.hoverTarget}refresh(){this.draw(this.hover,this.hoverTarget),this.draw(this.selected,this.selectedTarget)}clear(){this.setHover(null),this.setSelected(null)}draw(e,t){if(!t||!t.isConnected){e.hidden=!0;return}let r=t.getBoundingClientRect();e.hidden=!1,e.style.left=`${r.left}px`,e.style.top=`${r.top}px`,e.style.width=`${Math.max(r.width,0)}px`,e.style.height=`${Math.max(r.height,0)}px`,e.classList.toggle("edittrace-highlight--badge-below",r.top<28)}};function c(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function l(a){return c(a)}function d(a){return a?/^(https?:)?\/\//i.test(a)||a.startsWith("/"):!1}function w(a,e=90){return a.length>e?a.slice(0,e-1)+"\u2026":a}var g=class{constructor(e,t,r){this.config=t;this.callbacks=r;this.state={status:"idle"};this.element=document.createElement("aside"),this.element.className="edittrace-panel",this.element.setAttribute("role","complementary"),this.element.setAttribute("aria-label","EditTrace"),this.element.setAttribute("aria-live","polite"),this.element.setAttribute("tabindex","-1"),this.element.hidden=!0,e.appendChild(this.element),this.element.addEventListener("click",i=>this.handleClick(i))}getState(){return this.state}setState(e){if(this.state=e,this.element.hidden=e.status==="idle",this.element.innerHTML=this.render(e),e.status!=="idle"&&(this.element.scrollTop=0),e.status==="result"||e.status==="error")try{this.element.focus({preventScroll:!0})}catch{}}isOpen(){return this.state.status!=="idle"}close(){this.setState({status:"idle"})}handleClick(e){let t=e.target.closest("[data-edittrace-action]");if(!t)return;let r=t.getAttribute("data-edittrace-action");r==="close"?(e.preventDefault(),this.callbacks.onClose()):r==="parent"?(e.preventDefault(),this.callbacks.onInspectParent()):r==="search"&&(e.preventDefault(),this.callbacks.onSearch())}render(e){let t=`
			...
			<section class="edittrace-section edittrace-section--selected">
			...
			</section>`}renderResult(e,t,r){let i=[];if(i.push(this.selectedBlock(e.selected.summary,e.selected.tag)),e.status==="exact"&&e.primary){i.push(this.renderPrimary(e.primary));let n=e.candidates.slice(1);n.length&&i.push(this.renderOthers(n,"Also involved"))}else e.status==="candidates"?(i.push(`
				...
				</section>`),i.push(this.renderCandidateList(e.candidates))):i.push(this.renderUnknown(e,t,r));return t&&e.status!=="unknown"&&i.push('<div class="edittrace-loading"><span class="edittrace-spinner"></span>Searching WordPress\u2026</div>'),i.push(this.renderTechnical(e)),i.join("")}renderPrimary(e){let t=[];t.push(`<div class="edittrace-system">${c(e.system||e.provider)}</div>`),e.sourceName&&t.push(this.kv(e.sourceLabel||"Source",e.sourceName)),e.itemName&&t.push(this.kv(e.itemLabel||"Item",e.itemName,e.itemKey&&e.itemKey!==e.itemName?e.itemKey:""));for(let[i,n]of Object.entries(e.details||{}))t.push(this.kv(i,n));let r=`
			<section class="edittrace-section">
				...
				<section class="edittrace-section">
					...
					<ol class="edittrace-crumbs">${e.hierarchy.map((i,n)=>`<li class="edittrace-crumb${n===e.hierarchy.length-1?" edittrace-crumb--current":""}">${c(i)}</li>`).join("")}</ol>
				</section>`),e.global&&(r+=`
				<section class="edittrace-section">
				...
				<section class="edittrace-section">
					...
					${Object.entries(e.usage).map(([i,n])=>this.kv(i,String(n))).join("")}
			...
			</section>`}renderCandidateList(e){let t=e.slice(0,3),r=e.slice(3),i=o=>`
			...
			</li>`,n=`<ol class="edittrace-candidates">${t.map(i).join("")}</ol>`;return r.length&&(n+=`<details class="edittrace-details"><summary>${r.length} more</summary><ol class="edittrace-candidates" start="4">${r.map(i).join("")}</ol></details>`),n}renderUnknown(e,t,r){let i=this.config.fallbackSearch&&!e.searched;return`
			<section class="edittrace-section">
				...
				<div class="edittrace-actions">
					...
					${i&&!t?`<button type="button" class="edittrace-btn" data-edittrace-action="search">${c(this.config.i18n.search||"Search WordPress")}</button>`:""}
			...
			</section>`}renderTechnical(e){let t=[];t.push(this.kv("Current page",e.page.title||"\u2014")),e.page.template&&t.push(this.kv("Template",e.page.template)),e.page.postType&&t.push(this.kv("Queried object",`${e.page.postType} #${e.page.objectId}`)),t.push(this.kv("Page trace",e.trace.available?`available (${e.trace.entries} entries)`:"not available")),e.debug&&e.trace.id&&t.push(this.kv("Trace id",e.trace.id));let r=[...e.candidates,...e.weak||[]];for(let i of r){let n=Object.entries(i.technical||{}).filter(([o])=>e.debug||o!=="providerPriority").map(([o,s])=>`${o}: ${typeof s=="object"?JSON.stringify(s):String(s)}`).join(", ");t.push(this.kv(`${i.provider} (${i.confidence.toFixed(2)})`,`${i.sourceType}${i.sourceId?" "+i.sourceId:""}${n?" \u2014 "+n:""}`))}r.length||t.push(this.kv("Providers","no candidates"));for(let i of e.notes||[])t.push(`<div class="edittrace-note">${c(i)}</div>`);return e.selected.href&&t.push(this.kv("href",e.selected.href)),e.selected.src&&t.push(this.kv("src",e.selected.src)),`
			<details class="edittrace-details edittrace-technical">
				<summary>Technical details</summary>
				<div class="edittrace-technical__body">${t.join("")}</div>
			</details>`}confidenceBadge(e){return`<span class="edittrace-confidence edittrace-confidence--${l(e.status)}" title="Score ${e.confidence.toFixed(2)}">${c(e.statusLabel)}</span>`}kv(e,t,r=""){return`<div class="edittrace-kv"><span class="edittrace-kv__k">${c(e)}</span><span class="edittrace-kv__v">${c(t)}${r?`<span class="edittrace-kv__sub">${c(r)}</span>`:""}</span></div>`}};var R=new Set(["input","textarea","select","option","script","style","template","noscript"]);function k(a){return R.has(a.tagName.toLowerCase())}function D(a){if(k(a)){if(a.tagName.toLowerCase()==="input"){let o=(a.getAttribute("type")||"text").toLowerCase();if(o==="submit"||o==="button"||o==="reset")return(a.getAttribute("value")||"").trim()}return""}let e=[],t=document.createTreeWalker(a,NodeFilter.SHOW_TEXT|NodeFilter.SHOW_ELEMENT,{acceptNode(n){return n.nodeType===Node.ELEMENT_NODE?k(n)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_SKIP:NodeFilter.FILTER_ACCEPT}}),r=t.nextNode(),i=0;for(;r&&i<1e3*2;){let n=r.nodeValue||"";e.push(n),i+=n.length,r=t.nextNode()}return e.join(" ").replace(/\s+/g," ").trim().slice(0,1e3)}function I(a){let e=new Set;for(let t of a)(t.datasetKeys||[]).forEach(r=>e.add(r)),t.typeFromDataset&&e.add(t.typeFromDataset),t.typeFallbackDataset&&e.add(t.typeFallbackDataset);return e}function S(a,e){let t={},r=a.dataset;if(!r)return t;for(let i of Object.keys(r))if(i.startsWith("edittrace")||e.has(i)){let n=r[i];typeof n=="string"&&n.length<=200&&(t[i]=n)}return t}function _(a){return Array.from(a.classList).slice(0,40)}function F(a,e){return{tag:a.tagName.toLowerCase(),id:a.id||"",classes:_(a),dataset:S(a,e)}}function z(a){let e=null,t=a.tagName.toLowerCase();if(t==="img"?e=a:(t==="picture"||t==="figure")&&(e=a.querySelector("img")),!e)return{src:"",alt:"",width:0,height:0};let r=e.currentSrc||e.getAttribute("src")||e.getAttribute("data-src")||"";return{src:r.startsWith("data:")?"":r,alt:(e.getAttribute("alt")||"").trim(),width:e.naturalWidth||0,height:e.naturalHeight||0}}function v(a,e,t){let r=I(e),i=a.tagName.toLowerCase(),n=z(a),o="";(i==="a"||i==="area")&&(o=a.href||"");let s=[],h=a.parentElement;for(;h&&s.length<10&&h!==document.documentElement;)s.push(F(h,r)),h=h.parentElement;return{tag:i,text:D(a),href:o,src:n.src,alt:n.alt,width:n.width,height:n.height,id:a.id||"",classes:_(a),dataset:S(a,r),ancestors:s,pageUrl:window.location.href.split("#")[0],traceId:t}}var B=new Set(["span","strong","em","b","i","u","small","mark","sub","sup","code","abbr","time","s","del","ins","br","wbr","font","bdi","bdo","kbd","samp","var","cite","q","dfn"]),j=new Set(["a","button","h1","h2","h3","h4","h5","h6","p","li","label","td","th","blockquote","figcaption","dt","dd","summary","legend","pre"]),O=new Set(["a","button","h1","h2","h3","h4","h5","h6","p","li","ul","ol","img","picture","video","audio","figure","nav","header","footer","main","section","article","aside","form","table","blockquote","iframe","svg","input","select","textarea","label","details","summary"]),q=new Set(["script","style","template","noscript","link","meta"]),U={h1:"Heading",h2:"Heading",h3:"Heading",h4:"Heading",h5:"Heading",h6:"Heading",p:"Paragraph",a:"Link",button:"Button",img:"Image",picture:"Image",svg:"Icon",video:"Video",audio:"Audio",ul:"List",ol:"List",li:"List item",nav:"Navigation",header:"Header",footer:"Footer",main:"Main",section:"Section",article:"Article",aside:"Sidebar",form:"Form",table:"Table",tr:"Table row",td:"Table cell",th:"Table cell",blockquote:"Quote",figure:"Figure",figcaption:"Caption",iframe:"Embed",input:"Field",select:"Field",textarea:"Field",label:"Label",span:"Text",strong:"Text",em:"Text",div:"Container",details:"Details",summary:"Summary",time:"Text"};function C(a){return B.has(a.tagName.toLowerCase())}function $(a){return O.has(a.tagName.toLowerCase())}function W(a){return q.has(a.tagName.toLowerCase())}function x(a){let e=a.tagName.toLowerCase();if(e==="button")return!0;if(e==="input"){let r=(a.getAttribute("type")||"text").toLowerCase();return r==="submit"||r==="button"||r==="reset"}if(a.getAttribute("role")==="button")return!0;let t=a.className&&typeof a.className=="string"?a.className.toLowerCase():"";return/(^|[\s_-])(button|btn)([\s_-]|$)/.test(t)}function X(a){let e=a.tagName.toLowerCase();return e==="a"&&x(a)||e==="input"&&x(a)?"Button":U[e]||"Element"}function K(a,e){let t=a;for(;t&&t!==document.documentElement;){for(let r of e)try{if(t.matches(r.selector))return{element:t,marker:r}}catch{}t=t.parentElement}return null}function m(a,e){return e.some(t=>{try{return a.matches(t.selector)}catch{return!1}})}function T(a){return a.replace(/\.default$/,"").replace(/^core\//,"").replace(/^[a-z0-9-]+\//,"").split(/[-_./]/).filter(Boolean).map(e=>e.charAt(0).toUpperCase()+e.slice(1)).join(" ")}function G(a,e){var t;for(let r of[e.typeFromDataset,e.typeFallbackDataset]){if(!r)continue;let i=(t=a.dataset)==null?void 0:t[r];if(i)return T(i)}if(e.typeFromClassPrefix){for(let r of Array.from(a.classList))if(r.startsWith(e.typeFromClassPrefix)&&r.length>e.typeFromClassPrefix.length){let i=r.slice(e.typeFromClassPrefix.length);if(!/__|--/.test(i))return T(i)}}return""}function y(a,e){let t=K(a,e),r=X(a);if(!t)return r;let i=t.element===a&&G(a,t.marker)||r;return`${t.marker.label} \xB7 ${i}`}function b(a){return a.getBoundingClientRect()}function A(a,e,t=3){let r=b(a),i=b(e);return Math.abs(r.left-i.left)<=t&&Math.abs(r.top-i.top)<=t&&Math.abs(r.width-i.width)<=t&&Math.abs(r.height-i.height)<=t}function E(a,e){var i;let t=a,r=t.closest("svg");if(r&&r!==t&&(t=r),m(t,e))return t;if(C(t)){let n=t.parentElement;for(;n&&n!==document.body;){let o=n.tagName.toLowerCase();if(j.has(o)||m(n,e)||x(n))return n;if(!C(n))break;n=n.parentElement}}if(t.tagName.toLowerCase()==="source"&&((i=t.parentElement)==null?void 0:i.tagName.toLowerCase())==="picture"){let n=t.parentElement.querySelector("img");if(n)return n}return t}function N(a,e){let t=a.parentElement;for(;t&&t!==document.documentElement;){if(t===document.body)return null;if(m(t,e)||$(t)||!A(t,a))return t;t=t.parentElement}return null}function L(a){if(W(a))return!1;let e=b(a);return e.width>0&&e.height>0}function M(a,e,t){let r=Array.from(a.children).filter(L);if(r.length===0)return null;let i;t&&(i=r.find(o=>{let s=b(o);return t.x>=s.left&&t.x<=s.right&&t.y>=s.top&&t.y<=s.bottom}));let n=i||r[0];for(let o=0;o<6&&!(m(n,e)||$(n));o++){let s=Array.from(n.children).filter(L);if(s.length!==1||!A(s[0],n))break;n=s[0]}return n}var f=class{constructor(e,t){this.config=e;this.active=!1;this.lastPoint=null;this.raf=0;this.pendingHover=null;this.controller=null;this.requestId=0;this.launcher=null;this.api=new p(e),this.host=document.createElement("div"),this.host.id="edittrace-root",this.host.setAttribute("data-edittrace-ui",""),this.shadow=this.host.attachShadow({mode:"open"});let r=document.createElement("style");r.textContent=t,this.shadow.appendChild(r),this.highlighter=new u(this.shadow),this.toolbar=this.buildToolbar(),this.shadow.appendChild(this.toolbar),this.panel=new g(this.shadow,e,{onClose:()=>this.closePanel(),onInspectParent:()=>this.selectParent(),onSearch:()=>this.runSearch()}),this.globalStyle=document.createElement("style"),this.globalStyle.id="edittrace-global-style",this.globalStyle.textContent="html.edittrace-inspecting, html.edittrace-inspecting body, html.edittrace-inspecting body *:not([data-edittrace-ui]) { cursor: crosshair !important; }",this.onMouseMove=this.onMouseMove.bind(this),this.onClick=this.onClick.bind(this),this.onKeyDown=this.onKeyDown.bind(this),this.onScroll=this.onScroll.bind(this)}isActive(){return this.active}showLauncher(){if(this.launcher){this.launcher.hidden=this.active;return}this.host.isConnected||document.body.appendChild(this.host);let e=document.createElement("button");e.type="button",e.className="edittrace-launcher",e.setAttribute("aria-label","Activate EditTrace inspector"),e.title="EditTrace",e.textContent="EditTrace",e.addEventListener("click",()=>this.activate()),this.shadow.appendChild(e),this.launcher=e,document.addEventListener("edittrace:activate",()=>{e.hidden=!0}),document.addEventListener("edittrace:deactivate",()=>{e.hidden=!1})}toggle(){this.active?this.deactivate():this.activate()}activate(){this.active||(this.active=!0,this.host.isConnected||document.body.appendChild(this.host),document.head.appendChild(this.globalStyle),document.documentElement.classList.add("edittrace-inspecting"),this.host.classList.add("edittrace-root--active"),this.toolbar.hidden=!1,document.addEventListener("mousemove",this.onMouseMove,!0),document.addEventListener("click",this.onClick,!0),document.addEventListener("keydown",this.onKeyDown,!0),window.addEventListener("scroll",this.onScroll,!0),window.addEventListener("resize",this.onScroll),document.dispatchEvent(new CustomEvent("edittrace:activate")))}deactivate(){this.active&&(this.active=!1,this.cancelRequest(),document.documentElement.classList.remove("edittrace-inspecting"),this.globalStyle.remove(),this.host.classList.remove("edittrace-root--active"),this.toolbar.hidden=!0,this.highlighter.clear(),this.panel.close(),document.removeEventListener("mousemove",this.onMouseMove,!0),document.removeEventListener("click",this.onClick,!0),document.removeEventListener("keydown",this.onKeyDown,!0),window.removeEventListener("scroll",this.onScroll,!0),window.removeEventListener("resize",this.onScroll),document.dispatchEvent(new CustomEvent("edittrace:deactivate")))}getSelected(){return this.highlighter.getSelected()}async select(e){return this.highlighter.setSelected(e,y(e,this.config.markers)),this.highlighter.setHover(null),this.inspect(e)}isOwnNode(e){var r;return e instanceof Node?e.getRootNode()===this.shadow||this.host.contains(e)||!!((r=e.closest)!=null&&r.call(e,"#wpadminbar")):!1}onMouseMove(e){if(this.isOwnNode(e.target)){this.pendingHover=null,this.scheduleHover();return}this.lastPoint={x:e.clientX,y:e.clientY};let t=e.target instanceof Element?E(e.target,this.config.markers):null;this.pendingHover=t,this.scheduleHover()}scheduleHover(){this.raf||(this.raf=window.requestAnimationFrame(()=>{this.raf=0;let e=this.pendingHover;e&&e!==this.highlighter.getSelected()?this.highlighter.setHover(e,y(e,this.config.markers)):this.highlighter.setHover(null)}))}onClick(e){if(this.isOwnNode(e.target)||(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),!(e.target instanceof Element)))return;this.lastPoint={x:e.clientX,y:e.clientY};let t=E(e.target,this.config.markers);this.select(t)}onKeyDown(e){if(!this.active)return;let t=e.target;if(!(t&&!this.isOwnNode(t)&&/^(input|textarea|select)$/i.test(t.tagName)&&e.key!=="Escape"))switch(e.key){case"Escape":e.preventDefault(),e.stopPropagation(),this.panel.isOpen()?this.closePanel():this.deactivate();break;case"ArrowUp":e.preventDefault(),e.stopPropagation(),this.selectParent();break;case"ArrowDown":e.preventDefault(),e.stopPropagation(),this.selectChild();break;case"Enter":{let i=this.highlighter.getSelected()||this.highlighter.getHover();i&&(e.preventDefault(),e.stopPropagation(),this.select(i));break}default:break}}onScroll(){this.highlighter.refresh()}current(){return this.highlighter.getSelected()||this.highlighter.getHover()}selectParent(){let e=this.current();if(!e)return;let t=N(e,this.config.markers);t&&this.select(t)}selectChild(){let e=this.current();if(!e)return;let t=M(e,this.config.markers,this.lastPoint||void 0);t&&this.select(t)}closePanel(){this.cancelRequest(),this.panel.close(),this.highlighter.setSelected(null)}cancelRequest(){this.controller&&(this.controller.abort(),this.controller=null)}async inspect(e){this.cancelRequest();let t=++this.requestId,r=v(e,this.config.markers,this.config.traceId),i=r.text||r.alt||r.href||r.src||`<${r.tag}>`;this.panel.setState({status:"loading",summary:i}),this.controller=new AbortController;try{let n=await this.api.inspect(r,this.controller.signal);return t!==this.requestId?null:(this.panel.setState({status:"result",result:n,searching:!1}),document.dispatchEvent(new CustomEvent("edittrace:result",{detail:n})),n.status==="unknown"&&this.config.fallbackSearch&&this.runSearch(),n)}catch(n){return n.name==="AbortError"||t!==this.requestId||this.panel.setState({status:"error",message:n.message||this.config.i18n.error||"Request failed",summary:i}),null}}async runSearch(){let e=this.panel.getState(),t=this.highlighter.getSelected();if(e.status!=="result"||!t)return;let r=this.requestId;this.panel.setState({...e,searching:!0,searchError:void 0});let i=v(t,this.config.markers,this.config.traceId);this.controller=new AbortController;try{let n=await this.api.search(i,this.controller.signal);if(r!==this.requestId)return;let o=e.result,s={...o,status:n.candidates.length?"candidates":"unknown",primary:n.primary,candidates:n.candidates,weak:[...o.weak||[],...n.weak||[]],searched:!0,notes:[...o.notes||[],...n.notes||[]]};this.panel.setState({status:"result",result:s,searching:!1}),document.dispatchEvent(new CustomEvent("edittrace:result",{detail:s}))}catch(n){if(n.name==="AbortError"||r!==this.requestId)return;this.panel.setState({...e,searching:!1,searchError:n.message})}}buildToolbar(){let e=document.createElement("div");e.className="edittrace-toolbar",e.setAttribute("role","toolbar"),e.setAttribute("aria-label","EditTrace inspector"),e.hidden=!0;let t=this.config.i18n;return e.innerHTML=`
			...
			<button type="button" class="edittrace-toolbar__btn edittrace-toolbar__btn--exit" data-edittrace-tool="exit" title="Exit (Esc)">${t.exit||"Exit"}</button>`,e.addEventListener("click",r=>{let i=r.target.closest("[data-edittrace-tool]");if(!i)return;let n=i.getAttribute("data-edittrace-tool");n==="parent"?this.selectParent():n==="child"?this.selectChild():n==="exit"&&this.deactivate()}),e}};var P=`:host {
	all: initial;
	--et-bg: #16181d;
	--et-bg-2: #1e2128;
	--et-bg-3: #262a33;
...
`;function H(){let a=window.EditTraceConfig;if(!a||!a.restUrl||!a.traceId)return;let e=new f(a,P);window.EditTrace=e;let t=document.querySelector("#wp-admin-bar-edittrace > a");t&&(t.addEventListener("click",i=>{i.preventDefault(),e.toggle()}),document.addEventListener("edittrace:activate",()=>{var i;return(i=t.parentElement)==null?void 0:i.classList.add("edittrace-admin-bar--active")}),document.addEventListener("edittrace:deactivate",()=>{var i;return(i=t.parentElement)==null?void 0:i.classList.remove("edittrace-admin-bar--active")})),window.location.hash==="#edittrace"&&e.activate();let r=()=>{if(e.isActive())return;let i=t==null?void 0:t.getBoundingClientRect();!!i&&i.width>0&&i.height>0&&i.right<=window.innerWidth&&i.left>=0||e.showLauncher()};r(),window.addEventListener("resize",r)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",H):H();})();
